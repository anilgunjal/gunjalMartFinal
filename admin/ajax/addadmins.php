...
$date = date('Y-m-d H:i:s');
	...
	$message.= "Login on clicking this url : https://napreview.com/admin";
		...
		else
		{
			...
			{
				$stmt = $mysqli->prepare("INSERT INTO admins (name, empid, email, password, contact, address, dob, status, createDate ) VALUES (?,?,?,?,?,?,?,?,?)");
			...
			else
			{
				...
				if(file_exists("../uploads/admin/".$photo))
				{
					...
					$stmt = $mysqli->prepare("INSERT INTO admins (name, empid, email, password, contact, address, dob, avatar, status, createDate ) VALUES (?,?,?,?,?,?,?,?,?,?)");
